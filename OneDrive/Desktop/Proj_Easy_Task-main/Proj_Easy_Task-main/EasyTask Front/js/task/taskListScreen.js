...
let allPhases = [];
let allTasks = [];
let clients = []; // Adicionar variável global para clientes
let collaborators = []; // Adicionar variável global para colaboradores
...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        // 6. Carregar clientes e colaboradores
        await carregarClientesEColaboradores();
        
        // 7. Configurar event listeners
        configurarEventListeners();
        
        // 8. Carregar dados do quadro
        await carregarDadosDoQuadro();
        
        // 9. Carregar e renderizar tarefas
...
async function carregarDadosDoQuadro() {
    try {
        ...
        console.log("📥 Resposta do quadro:", boardResponse.status, boardResponse.statusText);
        
        if (!boardResponse.ok) {
            const errorText = await boardResponse.text();
            console.error("❌ Erro na resposta do quadro:", errorText);
            throw new Error(`Erro ao buscar quadro: ${boardResponse.status} - ${errorText}`);
        }
        ...
        console.log("📋 Buscando todas as fases...");
        const phasesResponse = await window.authManager.fetchWithAuth(
            `${window.API_CONFIG.BASE_URL}/phases`
        );
        
        console.log("📥 Resposta das fases:", phasesResponse.status, phasesResponse.statusText);
        
        if (!phasesResponse.ok) {
            const errorText = await phasesResponse.text();
            console.error("❌ Erro na resposta das fases:", errorText);
            throw new Error(`Erro ao buscar fases: ${phasesResponse.status} - ${errorText}`);
        }
        ...
        console.log("📋 Nomes das fases no quadro:", Array.from(phaseNamesOnBoard));
        
        const filteredPhases = allPhases.filter(phase => phaseNamesOnBoard.has(phase.name));
        console.log("📋 Fases filtradas:", filteredPhases);
        ...
        console.log("🏗️ Renderizando colunas do Kanban...");
        renderizarColunasKanban();
        
        console.log("✅ Carregamento dos dados do quadro concluído");
...
async function carregarTarefasDoQuadro() {
    try {
        ...
        console.log("📝 URL:", `${window.API_CONFIG.BASE_URL}/tasks/boards/${currentBoardId}`);
        ...
        console.log("🔧 Enriquecendo tarefas...");
...
async function enriquecerTarefasComIds(tarefas) {
    try {
        ...
        console.log("📋 Tarefas originais:", tarefas);
        ...
        const tarefasEnriquecidas = tarefas.map(tarefa => {
            ...
            // Usar o phaseId que já vem do backend (correto)
            const phaseId = tarefa.phaseId;
            
            const tarefaEnriquecida = {
                ...tarefa,
                clientId: clientId,
                collaboratorId: collaboratorId,
                phaseId: phaseId // Manter o phaseId original do backend
            };
            
            console.log(`🔧 Tarefa "${tarefa.title}":`, {
                original: { clientName: tarefa.clientName, collaboratorName: tarefa.collaboratorName, phaseName: tarefa.phaseName, phaseId: tarefa.phaseId },
                enriquecida: { clientId, collaboratorId, phaseId }
            });
            
            return tarefaEnriquecida;
        });
...
function renderizarColunasKanban() {
    console.log("🏗️ Renderizando colunas do Kanban...");
    console.log("🏗️ Fases do quadro:", boardPhases);
    ...
    console.log("🏗️ Container do Kanban encontrado:", kanbanBoard);
    ...
    console.log("🏗️ Fases ordenadas:", fasesOrdenadas);
    
    fasesOrdenadas.forEach((fase, index) => {
        console.log(`🏗️ Criando coluna ${index + 1}/${fasesOrdenadas.length} para fase:`, fase.name);
        const coluna = criarColunaFase(fase);
        kanbanBoard.appendChild(coluna);
        console.log(`✅ Coluna ${index + 1} adicionada ao Kanban`);
    });
    
    console.log("✅ Renderização das colunas concluída");
}

function criarColunaFase(fase) {
    console.log("🏗️ Criando coluna para fase:", fase.name, "ID:", fase.idPhase);
    
    const coluna = document.createElement('div');
    coluna.className = 'kanban-column';
    coluna.setAttribute('data-phase-id', fase.idPhase);
    // coluna.setAttribute('draggable', 'true'); // REMOVIDO: Apenas cards de tarefa são arrastáveis
    
    console.log("🏗️ Atributos da coluna:", {
        'data-phase-id': coluna.getAttribute('data-phase-id'),
        // 'draggable': coluna.getAttribute('draggable') // Não mais aplicável
    });
    ...
    console.log("🏗️ Container de tarefas criado:", {
        'data-phase-id': tarefasContainer.getAttribute('data-phase-id')
    });
    ...
    tarefasContainer.addEventListener('drop', async (e) => {
        ...
        console.log("🎯 Container:", tarefasContainer);
        ...
        if (!taskData || taskId.startsWith('phase-')) {
            console.log("⚠️ Drop ignorado - dados inválidos ou fase sendo arrastada");
            return;
        }
        ...
        console.log("🎯 Tarefa sendo movida:", tarefa);
        console.log("🎯 Nova fase ID:", novaFaseId);
        console.log("🎯 Fase atual da tarefa:", tarefa.phaseId);

        // Verificar se a tarefa está sendo movida para uma fase diferente
        if (tarefa.phaseId && novaFaseId !== tarefa.phaseId) {
            console.log(`🔄 Movendo tarefa ${tarefa.idTask} da fase ${tarefa.phaseId} para fase ${novaFaseId}`);
            await moverTarefaParaFase(tarefa.idTask, novaFaseId);
        } else {
            console.log(`ℹ️ Tarefa já está na fase ${novaFaseId}, não é necessário mover`);
        }
    });

    // NOVO: Listeners para a coluna inteira (abrange área vazia)
    ...
    coluna.addEventListener('drop', async (e) => {
        e.preventDefault();
        tarefasContainer.classList.remove('drag-over');
        // Redireciona o drop para o mesmo handler do tarefasContainer
        const taskId = e.dataTransfer.getData('text/plain');
        const taskData = e.dataTransfer.getData('application/json');
        if (!taskData || taskId.startsWith('phase-')) return;
        const tarefa = JSON.parse(taskData);
        const novaFaseId = fase.idPhase;
        if (tarefa.phaseId && novaFaseId !== tarefa.phaseId) {
            await moverTarefaParaFase(tarefa.idTask, novaFaseId);
    ...
    // Drag and drop para colunas (fases) - REMOVIDO
    // coluna.addEventListener('dragstart', ...)
    // coluna.addEventListener('dragend', ...)
    
    console.log("✅ Coluna criada com sucesso para fase:", fase.name);
    return coluna;
}
...
function renderizarTarefasNasColunas() {
    ...
    allTasks.forEach(tarefa => {
        console.log("🎯 Processando tarefa:", tarefa.title, "Fase ID:", tarefa.phaseId, "Fase Nome:", tarefa.phaseName);
        
        // Buscar a fase correspondente pelo ID (correto)
        const faseCorrespondente = boardPhases.find(fase => fase.idPhase === tarefa.phaseId);
        
        if (faseCorrespondente) {
            console.log("✅ Fase encontrada:", faseCorrespondente.name, "ID:", faseCorrespondente.idPhase);
            ...
            if (colunaElement) {
                console.log("✅ Coluna encontrada para fase:", faseCorrespondente.name);
                ...
                if (tarefasContainer) {
                    ...
                    console.log("✅ Card adicionado à coluna:", faseCorrespondente.name);
                } else {
                    console.warn("⚠️ Container de tarefas não encontrado na coluna:", faseCorrespondente.name);
                }
            } else {
                console.warn("⚠️ Coluna não encontrada para fase:", faseCorrespondente.name, "ID:", faseCorrespondente.idPhase);
                console.warn("🔍 Colunas disponíveis:", Array.from(document.querySelectorAll('.kanban-column')).map(c => ({ 
                    name: c.querySelector('.kanban-column-title')?.textContent, 
                    id: c.getAttribute('data-phase-id') 
                })));
            }
        } else {
            console.warn("⚠️ Fase não encontrada para tarefa:", tarefa.phaseName, "ID:", tarefa.phaseId);
            console.warn("🔍 Fases disponíveis:", boardPhases.map(f => ({ name: f.name, id: f.idPhase })));
        }
    });
...
function criarCardTarefa(tarefa) {
    console.log("🎴 Criando card para tarefa:", tarefa.title, "ID:", tarefa.idTask, "Fase ID:", tarefa.phaseId);
    ...
    console.log("🎴 Atributos do card:", {
        'data-task-id': card.getAttribute('data-task-id'),
        'data-phase-id': card.getAttribute('data-phase-id'),
    ...
    card.addEventListener('dragstart', (e) => {
        console.log("🎴 Drag start para tarefa:", tarefa.title, "ID:", tarefa.idTask);
        ...
        console.log("🎴 Dados transferidos:", {
            'text/plain': tarefa.idTask,
            'application/json': JSON.stringify(tarefa)
        });
    });
    card.addEventListener('dragend', (e) => {
        console.log("🎴 Drag end para tarefa:", tarefa.title);
    ...
    console.log("✅ Card criado com sucesso para tarefa:", tarefa.title);
    return card;
}
...
async function adicionarFaseAoQuadro(phaseId) {
    try {
        ...
        Swal.fire('Sucesso!', 'Estágio adicionado ao quadro com sucesso!', 'success');
        
    } catch (error) {
        console.error("❌ Erro ao adicionar fase:", error);
        Swal.fire('Erro', 'Não foi possível adicionar o estágio ao quadro', 'error');
    }
}
...
async function removerFaseDoQuadro(phaseId) {
    ...
    if (tarefasNaFase.length > 0) {
        Swal.fire({
            icon: 'error',
            title: 'Não é possível remover o estágio',
            text: `O estágio "${faseParaRemover.name}" contém ${tarefasNaFase.length} tarefa(s) e não pode ser desvinculado.`,
        });
    ...
    try {
        ...
        Swal.fire('Sucesso!', 'Estágio removido do quadro com sucesso!', 'success');
        
    } catch (error) {
        console.error("❌ Erro ao remover fase do quadro:", error);
        Swal.fire('Erro', 'Não foi possível remover o estágio do quadro', 'error');
    }
}
...
async function excluirFasePermanentemente(phaseId) {
    ...
    try {
        ...
        Swal.fire('Excluída!', 'O estágio foi excluído com sucesso.', 'success');
...
function abrirEdicaoFase(phaseId) {
    ...
    submitBtn.textContent = 'Salvar Estágio';
...
async function salvarFase(event) {
    ...
    try {
        ...
        const successMessage = isEditing ? 'Estágio atualizado com sucesso!' : 'Estágio criado com sucesso!';
        ...
        Swal.fire('Erro', 'Não foi possível salvar o estágio', 'error');
    }
}
...
async function abrirModalNovaTarefa() {
    try {
        ...
        if (boardPhases.length === 0) {
            console.warn("⚠️ Nenhuma fase vinculada ao quadro");
            kanbanBoard.innerHTML = `
                <div class="no-phases-message">
                    <i class="fas fa-exclamation-triangle"></i>
                    <h3>Nenhum estágio vinculado</h3>
                    <p>Este quadro ainda não possui estágios vinculados.</p>
                    <p>Entre em contato com o administrador para adicionar estágios ao quadro.</p>
                </div>
            `;
            return;
        }
...
async function carregarDadosParaFormularioTarefa() {
    try {
        ...
        if (!boardPhases || boardPhases.length === 0) {
            throw new Error('Nenhum estágio disponível no quadro');
        }
...
async function salvarTarefa(event) {
    ...
    try {
        ...
        if (!fase) {
            erros.push("Estágio é obrigatório");
        }
        ...
        if (!faseValida) {
            console.warn("⚠️ Estágio não pertence ao quadro atual");
            Swal.fire('Erro', 'O estágio selecionado não pertence a este quadro.', 'error');
            return;
        }
        ...
        if (!response.ok) {
            ...
            if (response.status === 403) {
                ...
                errorMessage = isEditingMode ? 'Tarefa não encontrada.' : 'Quadro, estágio, cliente ou colaborador não encontrado.';
            } else {
...
function configurarDragAndDropLixeira() {
    console.log("🗑️ Configurando drag and drop da lixeira...");
    
    const trashBin = document.getElementById('trashBin');
    if (!trashBin) {
        console.warn("⚠️ Lixeira não encontrada");
        return;
    }
    
    console.log("🗑️ Lixeira encontrada:", trashBin);
    
    trashBin.addEventListener('dragover', (e) => {
        e.preventDefault();
        trashBin.classList.add('drag-over');
        console.log("🗑️ Drag over na lixeira");
    });
    
    trashBin.addEventListener('dragleave', (e) => {
        if (!trashBin.contains(e.relatedTarget)) {
            trashBin.classList.remove('drag-over');
            console.log("🗑️ Drag leave na lixeira");
        }
    });
    ...
    trashBin.addEventListener('drop', async (e) => {
        ...
        console.log("🗑️ Drop na lixeira detectado");
        
        const dataType = e.dataTransfer.getData('text/plain');
        const jsonData = e.dataTransfer.getData('application/json');
        
        console.log("🗑️ Tipo de dados:", dataType);
        console.log("🗑️ Dados JSON:", jsonData);
        
        if (dataType && jsonData) {
            const data = JSON.parse(jsonData);
            console.log("🗑️ Dados parseados:", data);
            
            if (dataType.startsWith('phase-')) {
                console.log("🗑️ Tentando desvincular fase:", data.name);
                const result = await Swal.fire({
                ...
                if (result.isConfirmed) {
                    console.log("🗑️ Confirmado desvincular fase");
                    await removerFaseDoQuadro(data.idPhase);
                }
            } else {
                console.log("🗑️ Tentando excluir tarefa:", data.title);
                const result = await Swal.fire({
                    title: 'Excluir Tarefa?',
                ...
                if (result.isConfirmed) {
                    console.log("🗑️ Confirmado excluir tarefa");
                    await excluirTarefaPorId(data.idTask);
                }
            }
        } else {
            console.log("🗑️ Dados inválidos para drop na lixeira");
        }
    });
    
    console.log("✅ Drag and drop da lixeira configurado");
}
...
function verificarEventListenersDuplicados() {
    ...
    // Verificar botões de nova tarefa e novo estágio
    ...
    if (newTaskBtn) {
        ...
        const novoBtn = newTaskBtn.cloneNode(true);
        newTaskBtn.parentNode.replaceChild(novoBtn, newTaskBtn);
        novoBtn.addEventListener('click', abrirModalNovaTarefa);
        console.log("✅ Listener do botão nova tarefa configurado");
    } else {
        console.warn("⚠️ Botão nova tarefa não encontrado");
    }
    
    if (newCardBtn) {
        console.log("🔍 Botão novo estágio encontrado");
        // Remover listeners antigos e adicionar novos
        const novoBtn = newCardBtn.cloneNode(true);
        newCardBtn.parentNode.replaceChild(novoBtn, newCardBtn);
        novoBtn.addEventListener('click', abrirModalFases);
        console.log("✅ Listener do botão novo estágio configurado");
    } else {
        console.warn("⚠️ Botão novo estágio não encontrado");
    }
    
    // Verificar formulário de tarefa
    const tarefaForm = document.getElementById('tarefaForm');
    if (tarefaForm) {
        console.log("🔍 Formulário de tarefa encontrado");
        // Remover listeners antigos e adicionar novos
        const novoForm = tarefaForm.cloneNode(true);
        tarefaForm.parentNode.replaceChild(novoForm, tarefaForm);
        novoForm.addEventListener('submit', salvarTarefa);
        console.log("✅ Listener do formulário de tarefa configurado");
    } else {
        console.warn("⚠️ Formulário de tarefa não encontrado");
    }
    
    console.log("✅ Verificação de event listeners duplicados concluída");
}
...
async function moverTarefaParaFase(taskId, novaFaseId) {
    try {
        ...
        console.log(`📤 Enviando requisição para: ${window.API_CONFIG.BASE_URL}/tasks/${taskId}/move`);
        console.log(`📤 Corpo da requisição (apenas número):`, novaFaseId);
        ...
        const response = await window.authManager.fetchWithAuth(
            `${window.API_CONFIG.BASE_URL}/tasks/${taskId}/move`,
            {
                ...
                body: JSON.stringify(novaFaseId) // Enviar apenas o número, não um objeto
            }
        );
        ...
        if (!response.ok) {
            const errorText = await response.text();
            console.error(`❌ Erro na resposta:`, errorText);
            throw new Error(`Erro ao mover tarefa: ${response.status} - ${errorText}`);
        }
        
        const updatedTask = await response.json();
        console.log("✅ Tarefa movida com sucesso:", updatedTask);
...
async function carregarClientesEColaboradores() {
    try {
        console.log("👥 Carregando clientes e colaboradores...");
        
        // Carregar clientes e colaboradores em paralelo
        ...
        if (!collaboratorsResponse.ok) {
            const errorText = await collaboratorsResponse.text();
            console.error("❌ Erro na resposta dos colaboradores:", errorText);
            throw new Error(`Erro ao carregar colaboradores: ${collaboratorsResponse.status} - ${errorText}`);
        }
        ...
        console.log("✅ Carregamento de clientes e colaboradores concluído");
        
    } catch (error) {
        console.error("❌ Erro ao carregar clientes e colaboradores:", error);
        throw error;
    }
}